...
function onClickSearch() {
	...
	apiCall.get('/sysmgt/func/commoncodeI18N/searchGroupCd', params).then(res => {
...
function getSearchCommonCdList(gCode) {
	...
	apiCall
		.get('/sysmgt/func/commoncodeI18N/searchCommonCd', params)
		.then(res => {
...
function onClickSave() {
	//변경사항 체크
	if (!refCommonCodeGrid.value.isModified()) {
		message.showAlert({
			message: '변경사항이 없습니다.',
		});
		return false;
	...
	message.showConfirm({
		message: '저장하시겠습니까?',
		onConfirm: () => {
			let params = { i18nCodeDtls: refCommonCodeGrid.value.getChangedData() };
			console.log('getChangedData : ', params);
			apiCall.post('/sysmgt/func/commoncodeI18N/save', params).then(() => {
				message.showAlert({
					message: '저장되었습니다.',
				});
				onClickSearch();
