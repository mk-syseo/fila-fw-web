...
export default {
	...
	methods: {
		...
		getCurrentDragPercentage(drag) {
			drag = drag[this.vertical ? 'y' : 'x'];
			// In the code bellow 'size' refers to 'width' for vertical and 'height' for horizontal layout.
			const containerSize =
				this.container[this.vertical ? 'clientHeight' : 'clientWidth'];
			if (this.rtl && !this.vertical) drag = containerSize - drag;
	...
	render() {
		return h(
			'div',
			{
				...
				class: [
					'splitpanes',
					`splitpanes--${this.vertical ? 'horizontal' : 'vertical'}`,
					{
